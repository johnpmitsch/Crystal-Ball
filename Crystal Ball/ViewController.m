...
- (void)makePrediction {
    self.prediction.text = [self.crystalBall randomPrediction];
    [self.backgroundImageView startAnimating];
    [self.backgroundMusicPlayer play];
    [UIView animateWithDuration:3.0 animations:^{self.prediction.alpha = 1.0f;}];
...
- (void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    self.prediction.text = nil;
    self.prediction.alpha = 0;
}
